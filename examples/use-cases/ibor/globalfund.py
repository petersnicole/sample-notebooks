...
def valuation(api_factory, marketdata_scope, portfolio_group, time):
    ...
    dataframe = pd.concat([dataframe, dataframe.sum(numeric_only=True).to_frame().T], ignore_index=True)
    return dataframe
...
def create_portfolio_group(api_factory, scope, code, portfolios):
    ...
    portfolio_group = api_factory.build(
        lusid.api.PortfolioGroupsApi
    ).create_portfolio_group(scope=scope, create_portfolio_group_request=group_request)
...
def upsert_quotes(
    ...
    quote_type_values = {
        "mid_price": {"quote_type": "Price", "price_side": "Mid", "value": "price",},
        "mid_rate": {"quote_type": "Rate", "price_side": "Mid", "value": "rate",},
    }

    # Iterate over the quotes
    for index, quote in data_frame.iterrows():

        quote_type = quote_type_values[quote[required_mapping["quote_type"]]][
            "quote_type"
        ]
        ...
        for identifier in instrument_identifier_heirarchy:
            ...
            identifier_value = quote[
                instrument_identifier_mapping["identifier_mapping"][identifier]
            ]
        ...
        instrument_quotes[
            identifier + "_" + identifier_value + "_" + effective_date
...
def create_cut_labels(api_factory, exchange_names, cut_label_type):
    ...
    exchange_info = {
        "LSE": {"time_zone": "GB", "hour": 16, "minutes": 30},
        "NYSE": {"time_zone": "America/New_York", "hour": 16, "minutes": 0},
    }
    ...
    for exchange in exchange_names:
        ...
        try:
            response = api_factory.build(
                lusid.api.CutLabelDefinitionsApi
            ).get_cut_label_definition(code=exchange_code)
            response = api_factory.build(
                lusid.api.CutLabelDefinitionsApi
            ).delete_cut_label_definition(code=exchange_code)
        except lusid.ApiException as e:
            pass
    ...
    for exchange in exchange_names:
        exchange_code = exchange + "_" + cut_label_type
        exchange_time = models.CutLocalTime(
            exchange_info[exchange]["hour"], exchange_info[exchange]["minutes"]
        )
        ...
        try:
            response = api_factory.build(
                lusid.api.CutLabelDefinitionsApi
            ).create_cut_label_definition(create_cut_label_definition_request=request)
            responses.append(response)
        except lusid.ApiException as e:
            pass
